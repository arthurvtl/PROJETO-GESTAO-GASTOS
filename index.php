...
<main class="main">
    ...
    <div class="bodyArea">
        ...
        <?php for ($d = 0; $d < count($date); $d++) : ?>
            <div class="ItemBody">
                ...
                <div class="bodyTitle"><?= isset($date[$d]['titulo']) ? $date[$d]['titulo'] : 'N/A' ?></div>
                <!-- Exibe o tipo do item (Entrada ou Saída) -->
                <div class="bodyTitle"><?= isset($date[$d]['tipo']) ? $date[$d]['tipo'] : 'N/A' ?></div>
                <!-- Exibe a data do item formatada -->
                <div class="bodyTitle"><?= isset($date[$d]['data']) ? date('d/m/Y', strtotime($date[$d]['data'])) : 'N/A' ?></div>
                <!-- Exibe o valor do item formatado -->
                <div class="bodyTitle"><?= isset($date[$d]['valor']) ? number_format(str_replace(',', '', $date[$d]['valor']), 2, ',', '.') : 'N/A' ?></div>
    ...
    <div id="ModalArea" class="ModalArea">
        ...
        <form class="ModalFront" method="post" action="./add_item_action.php">
            ...
            <div class="label-float">
                <input inputmode="numeric" class="number" value="0,00" name="valor" required placeholder=" "/>
                <label>Valor</label>
            </div>
    ...
    <div id="ModalAreaEdit" class="ModalAreaEdit" style="display: <?= count($dataID) > 0 ? 'flex' : 'none' ?>;">
        ...
        <form class="ModalFrontEdit" method="post" action="./edit_item_action.php">
            ...
            <div class="label-float">
                <input value="<?= isset($dataID['titulo']) ? $dataID['titulo'] : '' ?>" type="text" autofocus name="novo_titulo" required placeholder=" "/>
                <label>Novo título</label>
            </div>
            <div class="rowInput">
                <div class="label-float-select">
                    <select name="novo_tipo">
                        <option value="Entrada" <?= isset($dataID['tipo']) && $dataID['tipo'] == 'Entrada' ? 'selected' : '' ?>>Entrada</option>
                        <option value="Saída" <?= isset($dataID['tipo']) && $dataID['tipo'] == 'Saída' ? 'selected' : '' ?>>Saída</option>
                    </select>
                </div>
                <div class="label-float">
                    <input value="<?= isset($dataID['data']) ? date('d/m/Y', strtotime($dataID['data'])) : '' ?>" id="newDate" type="text" name="newData" required placeholder=" "/>
                    <label>Nova data</label>
                </div>
            </div>
            <div class="label-float">
                <input inputmode="numeric" class="number" value="<?= isset($dataID['valor']) ? str_replace('.', ',', $dataID['valor']) : '' ?>" name="newValor" required placeholder=" "/>
